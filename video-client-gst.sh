...
config[videosink]="$5" ; if [ -z "${config[videosink]}" ] ; then config[videosink]=autovideosink ; fi
...
if [ ${config[video_port]/.*} -eq 0 -a ${config[audio_port]/.*} -eq 0 ] ; then
	...
	set -x
	gst-launch-1.0 videotestsrc is-live=true ! "video/x-raw,format=(string)I420,width=(int)640,height=(int)360,framerate=30/1" ! videoconvert ! ${config[videosink]}
	...
	# video only
	set -x
	gst-launch-1.0 ${gst[udpsrc]} port=${config[video_port]} caps="${config[video_caps]}" ! rtph264depay ! h264parse ! queue ! decodebin ! progressreport ! ${config[videosink]}
elif [ ${config[audio_port]/.*} -eq ${config[video_port]/.*} ] ; then
	# video+audio using same port (flvmux)
	set -x
	gst-launch-1.0 ${gst[udpsrc]} port=${config[video_port]} ! queue ! flvdemux name=mux mux.video ! "${config[video_caps]}" ! rtph264depay ! h264parse ! queue ! decodebin ! ${config[videosink]} mux.audio ! "${config[audio_caps]}" ! rtpmp4adepay ! "audio/mpeg,codec_data=(buffer)1290" ! queue ! decodebin ! audioconvert ! autoaudiosink
	...
	set -x
	gst-launch-1.0 ${gst[udpsrc]} port=${config[video_port]} caps="${config[video_caps]}" ! rtph264depay ! h264parse ! queue ! decodebin ! ${config[videosink]} & gst-launch-1.0 ${gst[udpsrc]} port=${config[audio_port]} caps="${config[audio_caps]}" ! rtpmp4adepay ! "audio/mpeg,codec_data=(buffer)1290" ! queue ! decodebin ! audioconvert ! autoaudiosink && fg
else
	# video+audio using separate ports and the same process (forces sync)
	set -x
	gst-launch-1.0 ${gst[udpsrc]} port=${config[video_port]} caps="${config[video_caps]}" ! rtph264depay ! h264parse ! queue ! decodebin ! ${config[videosink]} ${gst[udpsrc]} port=${config[audio_port]} caps="${config[audio_caps]}" ! rtpmp4adepay ! "audio/mpeg,codec_data=(buffer)1290" ! queue ! decodebin ! audioconvert ! autoaudiosink
fi
